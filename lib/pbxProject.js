...
pbxProject.prototype.addSourceFile = function (path, opt, group) {
    ...
    this.addToPbxSourcesBuildPhase(file, opt && opt.ext);       // PBXSourcesBuildPhase
...
pbxProject.prototype.addResourceFile = function(path, opt, group) {
    ...
    this.addToPbxBuildFileSection(file);        // PBXBuildFile
    this.addToPbxResourcesBuildPhase(file, opt.ext);     // PBXResourcesBuildPhase

    if (!opt.plugin) {
        this.addToPbxFileReferenceSection(file);    // PBXFileReference
        if (group) {
            var groupKey = this.findPBXGroupKey({name: group});
            this.addToPbxGroup(file, groupKey);         //Group other than Resources (i.e. 'splash')
        }
        else {
...
pbxProject.prototype.addFramework = function(fpath, opt) {
    ...
    if (this.hasFile(file.path) && !opt.ext) return false;
    ...
    if (link) {
      this.addToPbxFrameworksBuildPhase(file, opt.ext);    // PBXFrameworksBuildPhase
    }
...
pbxProject.prototype.addToPbxSourcesBuildPhase = function(file, ext) {
    var sources = this.pbxSourcesBuildPhaseObj(file.target, ext);
    sources.files.push(pbxBuildPhaseObj(file));
}
...
pbxProject.prototype.addToPbxResourcesBuildPhase = function(file, ext) {
    var sources = this.pbxResourcesBuildPhaseObj(file.target, ext);
    sources.files.push(pbxBuildPhaseObj(file));
}
...
pbxProject.prototype.addToPbxFrameworksBuildPhase = function(file, ext) {
    var sources = this.pbxFrameworksBuildPhaseObj(file.target, ext);
    sources.files.push(pbxBuildPhaseObj(file));
}
...
pbxProject.prototype.pbxSourcesBuildPhaseObj = function(target, ext) {
    return this.buildPhaseObject('PBXSourcesBuildPhase', (ext ? 'Extension ' : '') + 'Sources', target, true);
}

pbxProject.prototype.pbxResourcesBuildPhaseObj = function(target, ext) {
    return this.buildPhaseObject('PBXResourcesBuildPhase', (ext ? 'Extension ' : '') + 'Resources', target);
}

pbxProject.prototype.pbxFrameworksBuildPhaseObj = function(target, ext) {
    return this.buildPhaseObject('PBXFrameworksBuildPhase', (ext ? 'Extension ' : '') + 'Frameworks', target);
}
...
pbxProject.prototype.buildPhase = function(group, target, log) {
...
pbxProject.prototype.addTarget = function(name, type, subfolder) {
    ...
    var buildConfigurationsList = [
        {
            ...
            buildSettings: {
                ...
                CODE_SIGN_ENTITLEMENTS: path.join(targetSubfolder, targetSubfolder + '.entitlements'),
    ...
    var productName = targetName,
        ...
        productFile = this.addProductFile(productName, { group: 'Copy Files', 'target': targetUuid, 'explicitFileType': productFileType, 'lastKnownFileType': productFileType}),
    ...
    if (targetType === 'app_extension') {
        ...
        this.addBuildPhase([], 'PBXCopyFilesBuildPhase', 'Extension Copy Files', this.getFirstTarget().uuid,  targetType)

        this.addBuildPhase([], 'PBXSourcesBuildPhase', 'Extension Sources', targetUuid,  targetType)
        this.addBuildPhase([], 'PBXFrameworksBuildPhase', 'Extension Frameworks', targetUuid,  targetType)
        this.addBuildPhase([], 'PBXResourcesBuildPhase', 'Extension Resources', targetUuid,  targetType)

        // Add product to CopyFiles phase
        //this.addToPbxCopyfilesBuildPhase(productFile)
        var sources = this.buildPhaseObject('PBXCopyFilesBuildPhase', 'Extension Copy Files', productFile.target);
...
function correctForResourcesPath(file, project, group) {
    return correctForPath(file, project, group || 'Resources');
}
